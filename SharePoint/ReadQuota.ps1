...
{
    return $spSite.WebApplication.WebService.QuotaTemplates | Where-Object{ $_.QuotaID -eq $spSite.Quota.QuotaID }
}
...
function Get-SPApplicableQuota($spSite)
{
    ...
    if ($spSite.Quota.QuotaID -ne 0)
    {
        ...
        if ($null -ne $spSiteQuotaTemplate)
        {
...
function Get-SPQuotaInfo()
{
    ...
    $quotaTemplateColumn = @{name = 'QuotaTemplate'; expression = { $quotaID = $_.Quota.QuotaID; if ($quotaID -eq 0) { "No Template" } else { ($null -ne (Get-SPQuotaTemplate $_).Name, "Unknown ($($quotaID))")[0] } } }
    ...
    [Microsoft.SharePoint.SPSecurity]::RunWithElevatedPrivileges({
        ...
        $spQuotaInfo = $spSites | Select-Object Url, $quotaIDColumn, $quotaTemplateColumn, $storageMaximumLevelColumn, $StorageWarningLevelColumn, $storageUsedColumn
...
function Format-QuotaInfoTable($spQuotaInfo)
{
    ...
    $spQuotaInfo | Format-Table -AutoSize Url, $quotaTemplateDisplay, $storageMaximumLevelDisplay, $storageWarningLevelDisplay, $storageUsedDisplay, $percentQuotaUsedDisplay
}

$spQuotaInfo = Get-SPQuotaInfo | Sort-Object StorageUsed -Descending
